...
from py_bit_torrent.protocol import torrent_logger
...
class torrent_file_reader(torrent_metadata):
    ...
    def __init__(self, torrent_file_path):
        ...
        self.torrent_file_logger = torrent_logger.torrent_logger(
            "torrent file", torrent_logger.TORRENT_LOG_FILE, DEBUG
        )
        try:
        ...
        elif "announce" in self.torrent_file_extract.keys():
            trackers_url_list = [self.torrent_file_extract["announce"]]
        else:
            trackers_url_list = [None]
        ...
        if "files" in self.torrent_file_extract["info"].keys():
            ...
            files = [
                (file_data["length"], file_data["path"])
    ...
    def extract_torrent_metadata(self, torrent_file_raw_extract):
        ...
        for key, value in torrent_file_raw_extract.items():
            ...
            # if the current torrent file could have multiple files with paths
                ...
                torrent_extract[new_key] = list(
                    map(lambda x: self.extract_torrent_metadata(x), value)
                )
            ...
            # url list parameter
                    ...
                    type(value) == list
                    and new_key == "url-list"
                    or new_key == "collections"
            ):
                torrent_extract[new_key] = list(
                    map(lambda x: x.decode(self.encoding), value)
                )
            # if type of value is of type list
            elif type(value) == list:
                try:
                    torrent_extract[new_key] = list(
                        map(lambda x: x[0].decode(self.encoding), value)
                    )
                except:
    ...
    def __str__(self):
        ...
        if len(self.trackers_url_list) < 3:
            ...
            tracker_urls += "\n... "
            tracker_urls += (
                    str(len(self.trackers_url_list) - 1) + " more tracker urls !"
            )
