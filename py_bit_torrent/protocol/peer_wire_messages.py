...
from py_bit_torrent.protocol import torrent_error
...
class handshake:
    ...
    def validate_handshake(self, response_handshake):
        ...
        if response_handshake_length != HANDSHAKE_MESSAGE_LENGTH:
            err_msg = (
                "invalid handshake length ( " + str(response_handshake_length) + "B )"
            )
...
class bitfield(peer_wire_message):
    ...
    def __str__(self):
        message = "BITFIELD : "
        message += (
            "(message paylaod : [bitfield length : " + str(len(self.pieces_info)) + "])"
        )
        return message
