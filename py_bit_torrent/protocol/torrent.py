...
from py_bit_torrent.protocol.torrent_statistics import torrent_statistics
...
class torrent:
    ...
    def __init__(self, torrent_metadata, client_request, port: int | None = None, ip: str = "0.0.0.0"):
        ...
        self.client_port = port or random.randint(1024, 65535)
        self.client_IP = ip
        ...
        self.peer_id = (
                "-PC0001-" + "".join([str(rd.randint(0, 9)) for i in range(12)])
        ).encode()
    ...
    def get_piece_length(self, piece_index):
        ...
        if piece_index == self.pieces_count - 1:
            return (
                    self.torrent_metadata.file_size
                    - self.torrent_metadata.piece_length * (piece_index)
            )
        else:
    ...
    def __str__(self):
        ...
        # file name
        torrent_file_table.rows.append(
        ...
        torrent_file_table.rows.append(
            [
                "File size",
                str(round(self.torrent_metadata.file_size / (2 ** 20), 2)) + " MB",
            ]
        )
        # piece length
        torrent_file_table.rows.append(
        ...
        if self.torrent_metadata.files:
            torrent_file_table.rows.append(
                ["Files", str(len(self.torrent_metadata.files))]
            )
        else:
